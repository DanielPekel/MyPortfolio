<?php require_once 'header.php'; ?>

    <div class="views">
        <div class="container pt-md-1 pt-xl-4 pb-3">
            <div class="row justify-content-center justify-content-md-evenly pt-md-5 pb-3">
                <div class="col-3 d-none d-md-block text-center" style="padding-top:10%;">
                    <h1 class="fs-1 fw-bold text-white-50">Hi, I'm <span
                                class="head-primary">Daniël</span>
                    </h1>
                    <p class="fs-5 mt-3 fw-bolder text-white-50">I'm a <span
                                class="head-primary">software developer</span>
                        from the
                        Netherlands&nbsp</p>
                    <!-- TODO: mobile version text -->
                    <div class="container mt-md-5 mt-2">
                        <div class="row justify-content-center">
                            <div class="col-3 text-center">
                                <a class="fs-1" href="https://www.linkedin.com/in/dani%C3%ABl-pekel-software-dev/"><i
                                            class="fa-brands fa-linkedin"></i></a>
                            </div>
                            <div class="col-3 text-center">
                                <a class="fs-1 text-success" href="https://wa.me/684054534"><i
                                            class="fa-brands fa-whatsapp"></i></a>
                            </div>
                            <div class="col-3 text-center">
                                <a class="fs-1 text-white" href="mailto:danielpekel03@gmail.com"><i
                                            class="fa-solid fa-envelope"></i></a>
                            </div>
                            <div class="col-3 text-center">
                                <a class="fs-1 text-white" href="https://github.com/DanielPekel/"><i class="fa-brands fa-github"></i></a>
                            </div>
                        </div>
                    </div>
                    <div class="container mt-5 justify-content-center">
                        <a class="resumeButton" target="_blank" href="./assets/media/CV_Daniël_Pekel.pdf">Check out my resume</a>
                    </div>
                </div>
                <div class="col-8 text-start col-md-3 mt-5 pt-5 pt-md-0">
                    <img src="assets/media/64265.JPG" class="rounded-5" alt="A picture of myself" width="100%"
                         height="auto">
                </div>
            </div>
            <div class="pt-4 container text-center">
                <div class="container d-none d-md-block">
                    <div class="d-inline-flex">
                        <p id="bottomtext" class="w-0 fs-5 mt-3 fw-bolder typethis text-white-50">I'm a <span
                                    class="head-primary">software developer</span>
                            from the
                            Netherlands&nbsp</p>
                    </div>
                </div>
                <div class="container d-block d-md-none">
                    <div class="d-inline-flex">
                        <p id="bottomtextMobile1" class="w-0 fs-5 mt-3 fw-bolder typethis text-white-50">I'm a <span
                                    class="head-primary">software developer</span></p>
                    </div>
                </div>
                <div class="d-block d-md-none">
                    <div class="d-inline-flex">
                        <p id="bottomtextMobile2" class="w-0 fs-5 mt-3 fw-bolder typethis text-white-50">from the
                            Netherlands&nbsp</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="container mt-md-5 mt-2 d-block d-md-none">
            <div class="row justify-content-center">
                <div class="col-3 col-md-1 text-center">
                    <a class="fs-1" href="https://www.linkedin.com/in/dani%C3%ABl-pekel-software-dev/"><i
                                class="fa-brands fa-linkedin"></i></a>
                </div>
                <div class="col-3 col-md-1 text-center">
                    <a class="fs-1 text-success" href="https://wa.me/684054534"><i
                                class="fa-brands fa-whatsapp"></i></a>
                </div>
                <div class="col-3 col-md-1 text-center">
                    <a class="fs-1 text-white" href="mailto:danielpekel03@gmail.com"><i
                                class="fa-solid fa-envelope"></i></a>
                </div>
            </div>
        </div>
    </div>
    <div class="views pt-md-5">
        <div class="container pt-5">
            <div class="row pt-md-5 mt-md-5 justify-content-between">
                <div class="col-12 pt-5 mt-5 col-md-4">
                    <div class="topbar pt-1 px-3">
                        <div class="row justify-content-between">
                            <div class="col-md-5 col-8">
                                <p>Github Portfolio</p>
                            </div>
                            <div class="col-4">
                                <div class="container">
                                    <div class="row">
                                        <div class="col-4">
                                            <i class="fa-solid fa-up-right-and-down-left-from-center"></i>
                                        </div>
                                        <div class="col-4">
                                            <i class="fa-solid fa-minus"></i>
                                        </div>
                                        <div class="col-4">
                                            <i class="fa-solid fa-xmark"></i>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bottombar">
                        <div class="container pt-3">
                            <div class="container">
                                <div class="d-inline-flex">
                                    <p><code id="animate3" class="typethis"><span class="text-yellow fw-bold">git</span>
                                            init</code></p>
                                </div>
                            </div>
                            <div class="container">
                                <div class="d-inline-flex">
                                    <p><code id="animate4" class="typethis"><span>git</span> add .</code></p>
                                </div>
                            </div>
                            <div class="container">
                                <div class="d-inline-flex">
                                    <p><code id="animate5" class="typethis2"><span>git</span> remote add origin master
                                            <span
                                                    class="text-white-50"><a href="https://github.com/DanielPekel">https://github.com/DanielPekel</a></span></code>
                                    </p>
                                </div>
                            </div>
                            <div class="container">
                                <div class="d-inline-flex">
                                    <p><code id="animate6" class="typethis"><span>git</span> commit <span
                                                    class="text-white-50">-m</span>
                                            <span
                                                    class="text-blue">Pilot</span></code></p>
                                </div>
                            </div>
                            <div class="container">
                                <div class="d-inline-flex">
                                    <p><code id="animate7" class="typethis"><span>git</span> push</code></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-6 pt-5">
                    <div class="container mt-4">
                        <h2 class="text-white fw-bold pb-3">My projects</h2>
                        <p class="text-white-50">All of my projects are visible in <span class="text-primary">github</span>. I'm trying to expirement as
                            much as possible with
                            all different sorts of programming languages so I'll keep learning new things and habits in
                            coding.<br/><br/>
                            One of my latest projects is this portfolio website. I'm now learning to make an one page
                            website with as much necessary information for you.<br/><br/>
                            If you have a requests for a website or an app, I'm always open for a chat and I'll try
                            to get you
                            to reach your <span class="text-primary">goal</span>.
                        </p>
                    </div>
                    <h2></h2>
                </div>
            </div>
        </div>
    </div>
    <div class="views pt-md-5 pt-2">
        <div class="container pt-md-5 pt-0 px-4">
            <div class="row justify-content-between pt-md-5 mt-5">
                <div class="col-12 col-md-8 text-start pt-5 mt-3">
                    <h3 class="text-white fw-bold fs-1">About me</h3>
                    <p class="text-white-50 pt-4">
                        I'm a <?php
                        $birthdate = '2003-04-02';
                        $birthDate = new DateTime($birthdate);
                        $currentDate = new DateTime();
                        $age = $currentDate->diff($birthDate);
                        echo $age->y;
                        ?>-year-old software developer from the Netherlands. I'm
                        currently studying at Alfa College located in Hoogeveen.
                        I'm currently in my first year of the MBO ICT programme. I'm learning a lot about programming
                        and IT over all.<br/><br/>
                        I've worked at multiple companies so far, my experiences come from the following companies: <a
                                class="text-white attribute"
                                href="https://service-ict.nl">Service-ICT</a> and <a class="text-white attribute"
                                                                                     href="koelict.nl">Koel ICT</a>.
                        <br/><br/>
                    </p>
                    <div class="container text-center">
                        <p class="text-indigo fw-bold fs-4">"I'm hoping to become an experienced and
                            acknowledged<br/>full stack software developer in the future."
                        </p>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <img src="assets/media/portfolioImage2.jpg" class="rounded-5" alt="A picture of myself"
                         width="100%"
                         height="auto">
                </div>
            </div>
        </div>
    </div>
    <div class="views pt-5">
        <div class="container pt-5 text-center">
            <h2 class="text-white fw-bold fs-1">My skills</h2>
            <div class="row justify-content-center pt-md-5 mt-3 skillcards">
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-sharp fa-solid fa-share-nodes"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>Socializing</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-solid fa-brain"></i></div>
                            <div class="col-8 text-start"><h3>Innovative</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 fa-solid fa-user-group text-indigo"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>Team-player</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 fa-regular text-white fa-thumbs-up"></i></div>
                            <div class="col-8 text-start"><h3>Customer-friendly</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-regular fa-eye"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>Focused</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-github"></i></div>
                            <div class="col-8 text-start"><h3>Github</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-brands fa-html5"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>HTML</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-css3-alt"></i></div>
                            <div class="col-8 text-start"><h3>CSS</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-brands fa-js"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>Javascript</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><img src="assets/media/jquery_icon.png" width="40%" height="auto"/>
                            </div>
                            <div class="col-8 text-start"><h3>JQuery</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-brands fa-php"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>PHP</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-bootstrap"></i></div>
                            <div class="col-8 text-start"><h3>Bootstrap</h3></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="views pt-5">
        <div class="container pt-5 text-center">
            <h2 class="text-white pt-5 fw-bold fs-1">Skills I want to learn</h2>
            <div class="row justify-content-center pt-md-5 mt-3 skillcards">
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-brands fa-react"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>React</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-python"></i></div>
                            <div class="col-8 text-start"><h3>Python</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fa-brands fa-flutter text-indigo fs-1"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>Flutter</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-linux"></i></div>
                            <div class="col-8 text-start"><h3>Linux</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><img width="35%" height="auto" src="assets/media/csharp_icon.png"
                                                    alt="c sharp icon"/></div>
                            <div class="col-8 text-start text-indigo"><h3>C#</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><img width="35%" height="auto" src="assets/media/firebase_icon.webp"
                                                    alt="firebase icon"/></div>
                            <div class="col-8 text-start"><h3>Firebase</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-solid fa-database"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>NoSQL</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-java"></i></div>
                            <div class="col-8 text-start"><h3>Java</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-indigo fa-brands fa-golang"></i></div>
                            <div class="col-8 text-start text-indigo"><h3>GO</h3></div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="container">
                        <div class="row">
                            <div class="col-4"><i class="fs-1 text-white fa-brands fa-swift"></i></div>
                            <div class="col-8 text-start"><h3>Swift</h3></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="views pt-5">
        <div class="container pt-5 text-center">
            <h2 class="text-white pt-5 fw-bold fs-1">My hobbies</h2>
            <div class="row justify-content-center pt-md-5 mt-3">

                <div class="col-12 col-md-5">
                    <img src="assets/media/IMG_3299.JPG" width="80%" height="auto" alt="foto of me"
                         class="rounded-5"/>
                </div>
                <div class="col-12 col-md-7 pt-4 pt-md-0">
                    <p class="text-white-50">If I'm not coding you can find me at the gym or outside with my friends. I
                        started to
                        notice that exercise was
                        missing from my life, it gives me an energy rush and motivates me for days.<br/><br/>
                        I'm also a big fan of formula 1, one of my goals is to be at a race day.<br/><br/>
                        I also play guitar a bit.<br/><br/>
                        Besides all these hobbies one of them is coding, I did it as a hobby, and it eventually
                        turned into my job.</p>
                    <p class="text-indigo fw-bold fs-4">"If your hobby becomes your job, you will never work a day in
                        your life"
                    </p>

                </div>
            </div>
        </div>
    </div>
    <div class="views pt-5">
        <div class="container pt-5 text-center">
            <h2 class="text-white pt-5 fw-bold fs-1">Contact</h2>
            <div class="row justify-content-center py-md-5 mt-3">
                <div class="col-12 col-md-4">
                    <a href="tel:684054534" class="text-white-50 no-underline"><h4 class="fw-bold"><i
                                    class="fa-solid fa-phone"></i> +31 6 84054534</h4></a>
                </div>
                <div class="col-12 col-md-4">
                    <a href="mailto:danielpekel03@gmail.com" class="text-white-50 no-underline"><h4 class="fw-bold"><i
                                    class="fa-solid fa-envelope"></i> danielpekel03@gmail.com</h4></a>
                </div>
            </div>
            <div class="container my-md-5 mt-2">
                <div class="row justify-content-center pb-4">
                    <div class="col-3 col-md-1 text-center">
                        <a class="fs-1" href="https://www.linkedin.com/in/dani%C3%ABl-pekel-software-dev/"><i
                                    class="fa-brands fa-linkedin"></i></a>
                    </div>
                    <div class="col-3 col-md-1 text-center">
                        <a class="fs-1 text-success" href="https://wa.me/684054534"><i
                                    class="fa-brands fa-whatsapp"></i></a>
                    </div>
                    <div class="col-3 col-md-1 text-center">
                        <a class="fs-1 text-white" href="mailto:danielpekel03@gmail.com"><i
                                    class="fa-solid fa-envelope"></i></a>
                    </div>
                    <div class="col-3 col-md-1 text-center">
                        <a class="text-white fs-1" href="https://github.com/DanielPekel/"><i class="fa-brands fa-github"></i></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php require_once 'footer.php'; ?>